    ...
    def __init__(self, fname, tarcomponent):
        def variable_table(vname):
            table = np.array(
                [
                    ...
                    [
                        "xfVel",
                        "Horizontal velocity x-component at upper surface",
                        "m/yr",
                    ],
                    [
                        "yfVel",
                        "Horizontal velocity y-component at upper surface",
                        "m/yr",
                    ],
                    ["zfVel", "Vertical velocity at upper surface", "m/yr"],
                    [
                        "xbVel",
                        "Horizontal velocity x-component at lower surface",
                        "m/yr",
                    ],
                    [
                        "ybVel",
                        "Horizontal velocity y-component at lower surface",
                        "m/yr",
                    ],
                    ...
                    [
                        ...
                        "Mass balance at lower surface (active)",
                        "m/yr",
                    ],
                    [
                        "activeSurfaceThicknessSource",
                        "Mass balance at upper surface (active)",
                        "m/yr",
                    ],
                    [
        ...
        for level in range(n_level):
            ...
            for box in range(len(h5box)):
                x = (
                    np.arange(h5box["lo_i"][box] - 1, h5box["hi_i"][box] + 2) * dx
                    + dx / 2.0
                )
                y = (
                    np.arange(h5box["lo_j"][box] - 1, h5box["hi_j"][box] + 2) * dx
                    + dx / 2.0
                )
        ...
        for level in range(plotlevels):
            for box in range(self.boxes[level]):
                X, Y = np.meshgrid(
                    np.concatenate(
                        (
                            self.x[level][box] - self.dx[level] / 2.0,
                            [self.x[level][box][-1] + self.dx[level] / 2.0],
                        )
                    ),
                    np.concatenate(
                        (
                            self.y[level][box] - self.dx[level] / 2.0,
                            [self.y[level][box][-1] + self.dx[level] / 2.0],
                        )
                    ),
                )
...
def floatation(thick, bed, rhoi=918.0, rhoo=1028.0):
    ...
    for level in range(float.levels):
        for box in range(float.boxes[level]):
            float.data[level][box] = np.maximum(
                0.0,
                thick.data[level][box]
                + rhoo * np.minimum(0.0, bed.data[level][box]) / rhoi,
            )
    ...
    for level in range(ground.levels):
        for box in range(ground.boxes[level]):
            ...
            conditions = [
                ...
                (thck.data[level][box] == 0.0) & (bed.data[level][box] >= 0.0),
                (thck.data[level][box] > 0.0)
                & (
                    np.absolute(lsrf.data[level][box] - bed.data[level][box])
                    <= criterion
                ),
                (thck.data[level][box] > 0.0)
                & (
    ...
    for level in range(output.levels - 1):
        ...
        for box in range(output.boxes[level]):
            ...
            for upperbox in range(output.boxes[level + 1]):
                ...
                if (
                    (xlo <= output.x[level][box][-2])
                    & (xhi >= output.x[level][box][1])
                    & (ylo <= output.y[level][box][-2])
                    & (yhi >= output.y[level][box][1])
                ):
                    # if it does then work out area that overlaps and set mask

                    ilo = x2i(
                        max(xlo, output.x[level][box][1]) - output.x[level][box][0],
                        output.dx[level],
                    )
                    ihi = x2i(
                        min(xhi, output.x[level][box][-2]) - output.x[level][box][0],
                        output.dx[level],
                    )
                    jlo = x2i(
                        max(ylo, output.y[level][box][1]) - output.y[level][box][0],
                        output.dx[level],
                    )
                    jhi = x2i(
                        min(yhi, output.y[level][box][-2]) - output.y[level][box][0],
                        output.dx[level],
                    )

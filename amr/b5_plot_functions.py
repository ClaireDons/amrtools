...
def box_mesh(var, level, box):
    ...
    X, Y = np.meshgrid(
           np.concatenate((var.x[level][box] - var.dx[level]/2.0, [var.x[level][box][-1] + var.dx[level]/2.0])),
           np.concatenate((var.y[level][box] - var.dx[level]/2.0, [var.y[level][box][-1] + var.dx[level]/2.0])))
    return X, Y
...
def box_plot(var, level, box, ax, cmap, vmin, vmax):
    ...
    pcm = ax.pcolormesh(X, Y, var.data[level][box], cmap = cmap, 
                        vmin = vmin, vmax = vmax)
    return pcm
...
def box_rect(var, level, box, ax):
    ...
    color =['k', 'r', 'b', 'g', 'm', 'c']
    rect = ptch.Rectangle((var.x[level][box][0:2].sum()/2.0, 
                   var.y[level][box][0:2].sum()/2.0), 
...
def box_umod(u,v,level,box):
    ...
    umod = (u.data[level][box]*u.data[level][box] 
            + v.data[level][box]*v.data[level][box])**0.5
    return umod
...
def plot_umod(u,v,cmap, vmin, vmax):
    ...
    for level in range(u.levels):
      for box in range(u.boxes[level]):
        ...
        pcm = ax.pcolormesh(X, Y, umod, cmap = cmap, 
                        vmin = vmin, vmax = vmax)
    return fig, ax, pcm

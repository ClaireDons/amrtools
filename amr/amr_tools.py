...
class AMRfile:
    ...
    def nc2amr(self, nc2amrtool, var):
        ...
        name = self.find_name()
        amr = name + ".2d.hdf5"
        nc2amrOutput = subprocess.Popen(
            [nc2amrtool, self.file, amr, var], stdout=subprocess.PIPE
        )
        # assess
...
class h5amr:
    ...
    def get_names(self):
        ...
        names = [
            h5file.attrs["component_" + str(i)].decode("utf-8")
...
class statstool:
    ...
    def statsRun(self, driver, hdf5=""):
        ...
        statsCommand = (
            driver + " " + self.file + " 918 1028 9.81 " + hdf5 + " | grep time"
        )
...
class AMRfiles(statstool, h5amr):
    ...
    def lev0means(self):
        ...
        res = Parallel(n_jobs=2)(
            delayed(self.amrfile.get_varmeans)(f, df, n_components) for f in files
        )
    ...
    def stats(self, statsTool, hdf5=""):
        ...
        series_list = Parallel(n_jobs=num_jobs)(
            delayed(self.amrfile.statsRetrieve)(statsTool, i, df, hdf5) for i in files
        )
